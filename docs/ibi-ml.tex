...
\makeatother
\subtitle{Manuscript Draft for Fish and Fisheries Submission}
...
premiums. When charging a competitive market premium, contracts built
with random forest, regularized random forest, and support vector
machine algorithms were profitable for insurance. Thus, index insurance
is feasible in a private market, and provides fishers with protection
against environmental variability. However, bolstering fisher trust in
products built from complex machine learning algorithms remains a
significant step.
\end{abstract}
...
advanced fishery stock assessment models use little to no environmental
...
Fishery management is also unique challenge to navigate empirically that
is absent from agriculture insurance. Good fisheries management provides
...
collection, and increased exposure of fishers to environmental
variability, suggest a new opportunity has arisen to reconsider the use
of index insurance in fisheries. New methods including machine learning
algorithms could better capture the complex, nonlinear relationships
between weather and fishery productivity.
...
fair rates. When charging a competitive market premium, the contracts
built with random forest, regularized random forest, and support vector
machine algorithms were profitable for insurance companies. The leading
variable of influence in the nonlinear models included upwelling
measures, regional El Nino metrics, and krill abundance. Communicating
to fishers the relative influence of each variable is crucial to build
trust and transparency when using complex models. Overall, there is
potential for index insurance to be feasible in a private market, and
provide fishers with protection against environmental variability.
...
developing formal financial tools to manage fisheries risk has been
...
al.} 2024). Machine learning models can also improve estimation of crops
yields (Klompenburg \emph{et al.} 2020). Improved estimation of crop
yields leads to better insurance performance (Schmidt \emph{et al.}
...
well as demonstrating how imperfect prediction models lead to basis
risk. Section~\ref{sec-methods} describes the algorithms used to predict
fishery productivity, and how variables of importance were identified.
Section~\ref{sec-fish} describes the characteristics of the market squid
...
\(\eta\)\footnote{Fisheries scientists might argue that you should first
  ...
  short lifespans. Estimating their abundance is not available.
  Therefore the data does not exist in our context. Also insurance
  companies do not have access to the confidential data sources needed
  to build catch per unit effort and thus biomass estimates.}. Subbing
the estimator into Equation~\ref{eq-fishprod2} gives:
...
statistically significant to provide value to fishers. The model only
...
the level of protection offered by the contract. The main results of the
paper set coverage to 100\%, where any predicted catch below long run
average triggers a payout. In the appendix, multiple coverage levels are
tested at higher levels to mimic disaster level thresholds..
...
insurance contracts, and is suitable in this setting because of the
short time period of analysis (Jewson \emph{et al.} 2005).
...
number of years in the test set determines \(n_{test}\). The utility
function, \(u()\), is an increasing, concave function with risk
...
\(u_{ni}\), is the utility from the catch.
...
i.e.~\(\rho=\mathbb{E}[I(w)]\).

Second, I calculate the premium loading factor that makes fishers
indifferent between having insurance and not having insurance where
\(u_{rr}=0\) as \(m^*\). The premium loading factor is the multiple of
the expected payout that insurance companies charge to cover
administrative costs and profit. Varying \(m\) only affects the premiums
paid. The payout schedule is solely determined empirically by the
models. Values of \(m^*\) greater than 1 indicate fishers are willing to
pay more than the expected payout for insurance, whereas values below 1
indicate subsidies will be needed to stimulate demand.

\(m^*\) will be called the ``market'' premium as it represents the upper
bound an insurance company could charge for insurance. I calculate the
loss ratio, \(lr=\sum I(\omega)/\sum \rho\), at both actuarially fair
and market premiums to analyze the market viability of a contract. Loss
ratios above 1 indicate insurance companies lost money on the contract,
whereas loss ratios below 1 indicate insurance companies are profitable.
If a contract at \(m^*\) leads to \(lr<1\), then the contract is viable
in a free market. Insurers can use any \(m\) less than the market
\(m^*\) and still encourage fishers to purchase insurance.

\section{Methods}\label{sec-methods}

I use five models to predict yearly catch \(\hat{y}(w)\) that serves as
the index within Equation~\ref{eq-gamma}. Simple linear models are the
base model given its ubiquitous use in index insurance policies. Simple
linear models create single index contracts that use only one weather
variable. I refer to all simple linear regression models as
``univariate'' to differentiate between the multivariate models. The
four multivariate models will be LASSO regression, random forest,
regularized random forest, and support vector machine as predictor
models.
...
accurately calibrate each model. From the trained model, a single
prediction is made with environmental data from the next year. Whether
the contract pays out according to Equation~\ref{eq-gamma} is recorded.
The premium for that year is calculated using burn rate pricing on the
training set. In the next period, the training window rolls forward one
year to include 1990-2014. The model is retrained and a prediction is
made for 2015. This procedure continues until the end of the data set in
2023. Expected utility is the calculated for the test set based on the
payout schedule and premiums calculated as in Equation~\ref{eq-ut}.
...
Root mean squared error will serve as the metric to evaluate out of
sample predictive performance of each model in the hyperparameter
tuning.
...
implementation, simple linear models on single weather indices are the
most common design choice for index insurance policies (Benso \emph{et
al.} 2023). Regression models offer a basic starting place to consider
the viability of fishery index insurance.
...
Certain weather variables are likely to be highly correlated with each
other. I select the indices with the highest correlation to catch for
five categories of variables: Temperature, regional indices, upwelling,
prey, and paralarvae abundance.

Regularized random forest provides an agnostic, data driven methodology
for feature selection. Regularized random forest computes permutation
importance scores for each variable during out of bag testing. If the
...
added contingent on the permutation score. It is normalized from 0-1
with 1 acting as the most aggressive regularization. Models are built
with \texttt{grrf} package in R.

\subsection{Support Vector Macine}\label{support-vector-macine}

Support Vector Machine (SVM) constructs vectors that define an error
insenstive margin when used for regression. The algorithm then minimizes
a loss function on the slack points outside the support vectors. I use a
radial basis function kernel to map the predictors into a
higher-dimensional feature space, enabling the algorithm to capture
nonlinear patterns without explicitly defining transformations of the
input variables. Support Vector Machines work well in time series data
with low observations and high dimensionality (Sapankevych and Sankar
2009).

Model complexity is controlled by the regularization parameter, C, which
controls the width of the support vectors. A smaller C allows for a
wider margin, placing more data points within the support vectors and
enhances generalization. A larger C aims to minimize the error all
...
training examples. A small \(\gamma_{svm}\) implies a broader influence,
leading to less sensitivity to outliers, while a large \(\gamma_{svm}\)
focuses on nearby points, capturing more intricate patterns but risking
overfitting. I use a grid search method to tune C and \(\gamma_{svm}\)
to minimize RMSE in the test set. Models are built using the
\texttt{kernlab} package in R.
...
2005, the fishery was open access with fishers only paying for a licence
and a landing tax. The number of licensed fishers dramatically declined
in the implementation of the permit system in 2005. We adjust for the
change in permits by using the catch per fisher as the outcome variable
to allow comparison between pre-2005 and post-2005 management regimes.
...
resolution. Panel B is the proportion of historical market squid catch
...
per fisher and 12 weather variables. Catch histories extend to 1980, but
to ensure a balanced dataset I limit observations to 1990, which is the
earliest data available for squid and krill abundance in RREAS.
...
aggregated within the nearest California fishing block to provide an
annual time series.
...
RMSE for the multivariate models followed by random forests, and then
LASSO.
...
but each led to large losses for the insurance companies. Insurance
companies remained solvent with both random forest models, and fishers
...
Figure~\ref{fig-urm-lrm} shows the market premium an insurance company
could charge at the upper limit of demand for each model in Panel A. The
ensuing loss ratio at that market premium is shown in Panel B. Even at
the upper limit of demand, insurance companies would not be profitable
with the upwelling models and LASSO. The loss ratios remain above 1
implying insurance companies paid more out than they received at the
market premium. Fishers are most willing to pay for a contract with the
support vector machine as it has the highest premium. Insurance
...
positives helps insurance companies avoid excessive payouts, allowing
...
Next, I examine the payout and premium schedules over the testing period
to understand how contracts improve utility. Univariate models payout
less frequently (Figure~\ref{fig-pay} Right Panel). The krill index paid
out once during the lowest catch year in 2019. The large one, time
payout provides a significant gain in utility. Due to fisher risk
aversion, fishers are more willing to pay higher premiums to protect
against that single devastating loss (Figure~\ref{fig-prem}). The
upwelling indices pay more in later years. The frequency index pays out
every year from 2018-2023 with the largest payout in 2020 (197 mt per
fisher). These payout schedules by chance correspond to COVID-19
disruptions. However, the losses experienced by fishers are not driven
by weather risk, but rather a coincidence that the upwelling indices
predicted low catch during those years. The consistent large payouts
lead insurance companies to significantly increase premiums as shown in
Figure~\ref{fig-prem}.

Multivariate predictive models initiate more frequent payouts, but
payout larger quantities when fishers experience low catch
(Figure~\ref{fig-pay}). The flexibility of the underlying predictive
models drives the payout schedule. However, with larger payouts,
premiums must increase to compensate insurance suppliers. Premiums with
multivariate models are high and remain consistent throughout the ten
years of the test set even with updates each year
(Figure~\ref{fig-prem}). Contracts built with LASSO models differ the
most from the other multivariate models. LASSO contracts pay out large
sums in 2015 and 2023 when fishers experienced high catch. Insurance
companies suffer losses in those years limiting profits even at the
upper bound of market premiums.
...
determined by complicated, non-linear interactions. Frequency, ENSO, and
krill were consistently the most important variables in the non-linear
multivariate models Figure~\ref{fig-varimp}. Frequency became more
influential across all models in later years. Sea surface temperature
was important only within the random forests models.
...
random forest. All measures selected based on expertise were included
within the regularized random forest. The only missing information that
was valuable in the regularized random forest were PDO and ONI. Perhaps
...
similar patterns. Overall, the feature selection based on correlation
and expert knowledge was effective for both the random forest and SVM,
and did not exclude any of the primary weather variables.
...
PDO, CHCI, and SST are dropped from the model during some training
windows (Zoomed panel of Figure~\ref{fig-lassvip}).
...
at the commercial premiums (Jensen \emph{et al.} 2016). Single index
...
Models with high basis risk often lead to high premiums. However, in
this paper, the premiums fishers paid for multivariate models was higher
than single index models. This phenomena originates from a unique form
of data leakage that does not interfere with model performance, but does
influence premium calculation. Nonlinear machine learning models
accurately identify extreme losses in the training sets. Payouts in the
training set will be much higher leading to more expensive premiums.
Fishers are willing to pay these higher premiums as the multivariate
models accurately pay out large sums in bad times. However, this does
emphasize the need to avoid overfitting as inaccurate out of sample
predictions will lead fishers to pay high premiums without appropriate
compensation when needed. All index insurance contracts that plan to use
machine learning to identify weather to yield relationships will be
...
Diversifying the weather variable pool, spatially and ecologically, will
allow insurance companies to diversify their risk more seamlessly than
fishers are able. For example, insurance companies could create
...
react differently to weather variables (Free \emph{et al.} 2019, 2023).
...
on weather conditions four years prior.

An interesting consideration for the contract would be the timing of the
payout. Should the contract pay out when the weather event occurs, or
when fishers experience low catch? Paying out when the weather event
occurs would provide fishers with more time to adjust their fishing
strategies. However, the longer the lag between the weather event and
the payout, the more likely it is that other confounding factors could
influence catch.

Management provides the most interesting considerations. As demonstrated
in Section~\ref{sec-fish}, well managed fisheries often have the most
...
demonstrated reticence on using management measures as they perceive
...
Other triggers that originate from management could also be used.
Dungeness crab fishers stated interest in insuring against the number of
days the fishing season is delayed by management. Preseason Salmon run
strength predictions could be another. The exact nature and suitability
require rigorous assessment and cooperation with management to ensure
independence.
...
lack the expertise to navigate the complexity of fisheries. Partnerships
between fisheries scientists, economists, and insurance companies will
be essential to design effective contracts. As fishing communities face
increasing environmental and economic uncertainty, index insurance
provides a promising tool to help fishers remain financially resilient.
...
\begin{tabular}{lllll}
\toprule
\textbf{} & \textbf{Variable Name} & \textbf{Temporal Resolution} & \textbf{Spatial Resolution} & \textbf{Source}\\
\midrule
 & Sea Surface Temperature & Monthly & 5-km & NOAA Coral Bleaching Degree Heating Week\\
\cmidrule{2-5}
\multirow{-2}{*}{\raggedright\arraybackslash Temperature} & Cummulative Habitat Compression Index & Annual & 1-degree latitude & Integrated Ecosystem Assessment\\
\cmidrule{1-5}
...
\caption{\label{tbl-c80}Model performance and utility results when
coverage level of contract is set to 80\% of average catch. Instances
where no payouts are issued in the test set are indicated by ``No
payouts''. Loss ratios of market premiums that would need to be subsized
are indicated by ``-''.}
...
\toprule
Model & \% Utility & Loss Ratio & Train RMSE (mt per fisher) & $m^*$ & Loss Ratio at $m^*$\\
\midrule
SST & 29.1 & 1.17 & 228.7 & 1.1 & 1.079\\
Spring BEUTI & -4.2 & 0.00 & 226.3 & No payouts & No payouts\\
STI & -0.2 & 1.44 & 243.4 & 0.3 & -\\
Frequency & 63.5 & 1.53 & 226.1 & 1.5 & 1.013\\
Relaxation & -0.4 & 0.00 & 238.0 & No payouts & No payouts\\
\addlinespace
Spring CUTI & 0.0 & 0.00 & 236.1 & No payouts & No payouts\\
CHCI & -6.5 & 0.48 & 224.6 & 0 & -\\
ENSO & -11.1 & 0.82 & 221.8 & 0.5 & -\\
PDO & -12.1 & 0.65 & 233.5 & 0.1 & -\\
ONI & -1.3 & 0.94 & 224.0 & 0.9 & -\\
\addlinespace
Krill & 0.0 & NaN & 216.0 & NA & NA\\
Squid & -3.4 & 0.00 & 237.1 & No payouts & No payouts\\
Random Forest & -3.1 & 0.00 & 115.8 & No payouts & No payouts\\
Regularized RF & -2.3 & 0.00 & 133.6 & No payouts & No payouts\\
Support Vector Machine & 3.4 & 0.45 & 144.7 & 1.8 & 0.253\\
\addlinespace
LASSO & 19.9 & 2.84 & 130.2 & 2.9 & 0.963\\
\bottomrule
\end{tabular}}
...
\caption{\label{tbl-c90}Model performance and utility results when
coverage level of contract is set to 90\% of average catch. Instances
where no payouts are issued in the test set are indicated by ``No
payouts''. Loss ratios of market premiums that would need to be subsized
are indicated by ``-''.}
...
Model & \% Utility & Loss Ratio & Train RMSE (mt per fisher) & $m^*$ & Loss Ratio at $m^*$\\
\midrule
SST & 29.1 & 1.59 & 228.7 & 1.3 & 1.25\\
Spring BEUTI & -10.4 & 0.00 & 226.3 & No payouts & No payouts\\
STI & -0.3 & 1.64 & 243.4 & 0.3 & -\\
Frequency & 58.7 & 2.68 & 226.1 & 2.6 & 1.046\\
Relaxation & 6.5 & 3.07 & 238.0 & 2.8 & 1.112\\
\addlinespace
Spring CUTI & -2.6 & 0.00 & 236.1 & No payouts & No payouts\\
CHCI & -14.4 & 0.61 & 224.6 & 0.1 & -\\
ENSO & -12.9 & 0.75 & 221.8 & 0.4 & -\\
PDO & -12.1 & 0.52 & 233.5 & 0.1 & -\\
ONI & -3.7 & 0.92 & 224.0 & 0.8 & -\\
\addlinespace
Krill & -4.6 & 0.00 & 216.0 & No payouts & No payouts\\
Squid & -4.5 & 0.00 & 237.1 & No payouts & No payouts\\
Random Forest & 10.3 & 0.84 & 115.8 & 1.4 & 0.596\\
Regularized RF & -1.6 & 0.61 & 133.6 & 0.9 & -\\
Support Vector Machine & 21.4 & 1.27 & 144.7 & 1.8 & 0.716\\
\addlinespace
LASSO & 30.2 & 1.78 & 130.2 & 1.9 & 0.956\\
\bottomrule
\end{tabular}}
...
\caption{\label{tbl-log}Model performance and utility results with log
utility. Instances where no payouts are issued in the test set are
indicated by ``No payouts''. Loss ratios of market premiums that would
need to be subsized are indicated by ``-''.}
...
Model & \% Utility & Loss Ratio & Train RMSE (mt per fisher) & $m^*$ & Loss Ratio at $m^*$\\
\midrule
SST & 0.2 & 2.66 & 228.7 & 1.3 & 2.128\\
Spring BEUTI & -0.7 & 0.07 & 226.3 & 0.2 & -\\
STI & 0.0 & 1.64 & 243.4 & 1.1 & 1.478\\
Frequency & 2.4 & 3.96 & 226.1 & 4 & 1.003\\
Relaxation & 0.7 & 2.91 & 238.0 & 2.8 & 1.052\\
\addlinespace
Spring CUTI & -0.4 & 0.00 & 236.1 & No payouts & No payouts\\
CHCI & -0.6 & 0.64 & 224.6 & 0.4 & -\\
ENSO & -0.5 & 0.71 & 221.8 & 0.5 & -\\
PDO & -0.5 & 0.45 & 233.5 & 0.2 & -\\
ONI & -0.1 & 0.89 & 224.0 & 0.9 & -\\
\addlinespace
Krill & -0.1 & 0.55 & 216.0 & 0.9 & -\\
Squid & -0.3 & 0.00 & 237.1 & No payouts & No payouts\\
Random Forest & 0.4 & 1.04 & 115.8 & 1.2 & 0.895\\
Regularized RF & 0.1 & 1.00 & 133.6 & 1.1 & 0.947\\
Support Vector Machine & 1.4 & 1.38 & 144.7 & 1.5 & 0.895\\
\addlinespace
LASSO & 2.0 & 1.68 & 130.2 & 1.6 & 1.028\\
\bottomrule
\end{tabular}}
...
Free, C.M., Anderson, S.C., Hellmers, E.A., et al. (2023)
\href{https://doi.org/10.1111/faf.12753}{Impact of the 2014--2016 marine
heatwave on US and canada west coast fisheries: Surprises and lessons
from key case studies}. \emph{Fish and Fisheries} \textbf{24}, 652--674.
...
\bibitem[\citeproctext]{ref-Jewson2005}
Jewson, S., Brix, A. and Ziehmann, C. (2005) \emph{Weather derivative
valuation: The meteorological, statistical, financial, and mathematical
...
\bibitem[\citeproctext]{ref-Sapankevych2009}
Sapankevych, N. and Sankar, R. (2009)
\href{https://doi.org/10.1109/MCI.2009.932254}{Time series prediction
using support vector machines: A survey}. \emph{IEEE Computational
Intelligence Magazine} \textbf{4}, 24--38.
