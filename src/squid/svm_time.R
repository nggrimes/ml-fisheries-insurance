...
time_svm_df<-svm_grid %>% 
  mutate(results=pmap(across(everything(.)),~tune_map(...,v='blank',coverage=1,m=1,fv='mt_per_detrend',big_data=port_cw$cw_data[[1]],ra=0.01,ut_mod='log',pred_mod='svm'))) |> 
...
max_len <- max(length(df$fish_value), length(best_svm$train_vec[[1]]), length(best_svm$test_vec[[1]]))

# Pad each vector
fish_value <- c(df$fish_value, rep(NA, max_len - length(df$fish_value)))
train <- c(best_svm$train_vec[[1]][[1]], rep(NA, max_len - length(best_svm$train_vec[[1]][[1]])))
test <- c(rep(NA, max_len - length(best_svm$test_vec[[1]][[1]])),best_svm$test_vec[[1]][[1]])
