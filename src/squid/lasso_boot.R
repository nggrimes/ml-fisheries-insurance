...
ut_i<-ut_fcn(value=lasso_y_test,payout_vec=payout_out_test,premium=prem,ra=0.1,mod='log')
ut_noi<-ut_fcn(value=lasso_y_test,payout_vec=0,premium=0,ra=0.1,mod='log')

u_rr<-(ut_i$ut-ut_noi$ut)/abs(ut_noi$ut)
...
boot_lasso<-function(fv,big_data,coverage=1,m=1,split_t=25,ra,ut_mod){
  ...
  return(data.frame(
    ut_i=mean(boot_out$ut_i,na.rm=TRUE),
    ut_noi=mean(boot_out$ut_noi,na.rm=TRUE),
