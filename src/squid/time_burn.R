...
analysis<-function(index,data,coverage,m,ra,ut_mod,pred_mod){
  ...
  if(pred_mod=='lm')
  {
    ...
    exclude_vars <- c("year", "fish_var","oni","chci","pdo","krill","sti","relax","sp_cuti")
    ...
    rf_mod <- ranger::ranger(formula=form, data = train, importance = 'impurity', num.trees = 1000, mtry = 3, min.node.size = 7,max.depth = 3)
  ...
  # return results
  return(data.frame(payout_vec_test=payout_vec_test,
                    rsq=rsq,
                    prem=prem,
                    rmse=rmse,
...
time_step<-function(v,fv,big_data,coverage=1,m=1,start_year=24,ra,ut_mod,pred_mod){
  ...
  time_out<-vec_list %>% map_df(.f=~analysis(.x,data=df,coverage=coverage,m=m,ut_mod=ut_mod,ra=ra,pred_mod=pred_mod))
...
time_lm_df<-combo %>% 
  mutate(results=map2(.x=var_names,.y=fish_vars,~time_step(.x,.y,big_data=port_cw$cw_data[[1]],ra=0.01,ut_mod='log',pred_mod='lm'))) |> 
  unnest_wider(results)


time_las_df<-combo_las %>% 
  mutate(results=map2(.x=mods,.y=fish_vars,~time_step(v='blank',.y,big_data=port_cw$cw_data[[1]],ra=0.01,ut_mod='log',pred_mod=.x))) |> 
  unnest_wider(results)
