...
out<-cali_cw %>% 
  mutate(sst_mt_lm=map(.x=cw_data,~lm_mod_fcn(x="avg_sst",y="value_usd",data=.x))) |> 
  mutate(sst_mt_pred=map2(.x=cw_data,.y=sst_mt_lm,~pred_fcn(input="avg_sst",output="value_usd",.x,.y))) |> 
  mutate(pre_ins_pay=map2(.x=data,.y=sst_mt_pred,~pre_ins_pay(.x,.y))) |> 
  mutate(post_ins_pay=map2(.x=data,.y=sst_mt_pred,~post_ins_pay(.x,.y))) |> 
  mutate(avg_ins_pay=map2(.x=data,.y=sst_mt_pred,~avg_ins_pay(.x,.y))) |>
  ...
  mutate(ut_pre=pmap(list(data,pre_ins_pay,premium_pre),utility_ins,model="isoelastic",par=1)) |> 
  mutate(ut_noins=map(data,utility,model="isoelastic",par=1)) |> 
  mutate(ut_post=pmap(list(data,post_ins_pay,premium_post),utility_ins,model="isoelastic",par=1)) |> 
  mutate(ut_avg=pmap(list(data,avg_ins_pay,premium_avg),utility_ins,model="isoelastic",par=1)) |> 
...
gg_fcn<-function(data,pred){
  data<-data[[1]]
  
  pred_df<-data.frame(year=seq(1988,2023),pred=pred[[1]])
  data |> 
    ggplot()+
    geom_line(aes(x=year,y=value_usd,color="Landings"))+
